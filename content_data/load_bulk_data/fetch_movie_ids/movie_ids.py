"""
This file contains the functionality to fetch movie_ids from TMDB.
...
CONFIG = Config()
logger = Logger('run_raw_movies').get_logger()

class MovieIDs:
    """
    This class contains methods to fetch movie_ids from TMDB.
    It uses the Config() module to access configuration and endpoint settings for TMDB.
    It sets configurations as follows:

        - `self.url`: TMDB API endpoint for fetching movies.
        - `self.headers`: TMDB API headers.
        - `self.default_params`: TMDB API default parameters.
        - `self.page`: Page number for fetching movies.
        - `self.start_date`: Start date for fetching movies.
        - `self.end_date`: End date for fetching movies.
        - `self._set_params()`: Sets the parameters for fetching movies.
        - `self.total_page_params`: Parameters for fetching total pages.
        - `self.dynamic_params`: Parameters for fetching movies dynamically.
    ...
    #### Example Usage:
        ...
        >>> raw_movies = RawMovies(page=1, start_date="2020-01-01", end_date="2020-12-31")
        >>> total_pages = raw_movies.get_total_pages()
        >>> movies = raw_movies.fetch_movies()
    """
    def __init__(self, page: int = None, start_date: str = None, end_date: str = None):
        self.url, self.headers, self.default_params = CONFIG.get_tmdb_config(endpoint="discover", type="movies")
        self.page = page
    ...
    def fetch_movies(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            ...
            List[Dict[str, Any]]: List of movies data.
